...
locals {
  ...
  location_shortcode                            = lookup(local.location_shortcode_map, local.location, substr(local.location, 0, 4))
  ...
  aks_user_assigned_identity_name               = "aks-${var.application_name}-${local.environment}-${local.location_shortcode}-${var.aks_name_suffix}-id"
  aks_kubelet_identity_name                     = "aks-${var.application_name}-${local.environment}-${local.location_shortcode}-${var.aks_name_suffix}-kubelet"
}